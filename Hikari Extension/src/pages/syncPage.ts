...
export class SyncPage {
  ...
  private getHikariMatchKey(state: pageState) {
    const overviewUrl = this.page.sync?.getOverviewUrl?.(this.url);
    if (overviewUrl) return overviewUrl;
    if (state.identifier) return `${this.page.name}:${state.identifier}`;
    return `${this.page.name}:${state.title}`;
  }

  private async resolveHikariMedia(state: pageState) {
    const title = cleanupTitle(state.title) || state.title;
    const cacheKey = this.getHikariMatchKey(state);

    if (cacheKey) {
      const cachedId = await getCachedMatch(cacheKey);
      if (cachedId) {
        try {
          const cachedMedia = await fetchMediaById(cachedId);
          if (cachedMedia) return cachedMedia;
        } catch (error) {
          logger.warn('[Hikari] Cached media lookup failed', error);
        }
      }
    }

    const candidates = [title, state.title]
      .concat(
        state.identifier
          ? [cleanupTitle(state.identifier.replace(/[-_]+/g, ' ')) || state.identifier]
          : [],
      )
    ...
    for (const candidate of candidates) {
      ...
      seen.add(candidate);
      const results = await searchAniList(candidate);
      const best = pickBestMatch(results, candidate);
      if (best) {
        if (cacheKey) {
          await cacheMatch(cacheKey, best.id);
        }
        return best;
      }
    }
  ...
  private async syncHikari(state: pageState) {
    ...
    if (!session) {
      const warnKey = `hikari:auth:${this.page.name}`;
      if (!localStore.getItem(warnKey)) {
    ...
    try {
      if (info.action === 'presence') {
        ...
        if (!presenceHidePage) {
          ...
          largeImageTextTemp = `${this.page.name} • Hikari`;
        } else {
          largeImageKeyTemp = 'malsync';
          largeImageTextTemp = 'Hikari';
        }
