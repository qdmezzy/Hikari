...
const sourceMockUser = {
  ...
  bio: "Anime enthusiast and manga collector. Currently on a quest to watch every Ghibli film. Love slice of life and fantasy genres! Let's be friends and share recommendations!",
  ...
  recentActivity: [
    { anime: "Frieren: Beyond Journey's End", episode: 24, status: "completed", image: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx154587-n1fmjRv4JQUd.jpg", score: 10, time: "2 hours ago" },
    { anime: "Solo Leveling", episode: 8, status: "watching", image: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx151807-m1gX3iwfIsLu.png", score: null, time: "5 hours ago" },
    { anime: "Dungeon Meshi", episode: 12, status: "watching", image: "https://s4.anilist.co/file/anilistcdn/media/anime/cover/large/bx153518-D1bkT82xtUoC.jpg", score: null, time: "Yesterday" },
  ],
  favorites: [
  ...
  achievements: [
    { name: "Century Club", desc: "Completed 100 anime", icon: Trophy, color: "text-amber-500", unlocked: true },
    { name: "Binge Master", desc: "Watched 20 episodes in a day", icon: Zap, color: "text-purple-500", unlocked: true },
    { name: "Critic", desc: "Rated 50 anime", icon: Star, color: "text-yellow-500", unlocked: true },
    { name: "Dedicated", desc: "14 day streak", icon: Flame, color: "text-orange-500", unlocked: true },
    { name: "Explorer", desc: "Watched anime from 10 genres", icon: Globe, color: "text-blue-500", unlocked: true },
    { name: "Elite", desc: "Reach level 50", icon: Crown, color: "text-gray-400", unlocked: false },
  ]
}
...
export default function ProfilePage() {
  ...
  const [settings, setSettings] = React.useState({
    displayName: sourceMockUser.name,
    username: sourceMockUser.username,
    bio: sourceMockUser.bio,
    location: sourceMockUser.location,
    website: sourceMockUser.website,
  ...
  const displayName = user?.user_metadata?.display_name || user?.user_metadata?.full_name || user?.email || sourceMockUser.name
  const rawHandle =
    user?.user_metadata?.handle ||
    user?.user_metadata?.username ||
    (user?.email ? user.email.split("@")[0] : sourceMockUser.username)
  const normalizedHandle = normalizeHandle(rawHandle) || sourceMockUser.username
  const avatarUrl = user?.user_metadata?.avatar_url || user?.user_metadata?.avatar || sourceMockUser.avatar
  const bannerUrl = user?.user_metadata?.banner_url || sourceMockUser.banner
  const bio = user?.user_metadata?.bio || sourceMockUser.bio
  const location = user?.user_metadata?.location || sourceMockUser.location
  const website = user?.user_metadata?.website || sourceMockUser.website
  const joinedDate = user?.created_at
    ? new Date(user.created_at).toLocaleDateString("en-US", { month: "long", year: "numeric" })
    : sourceMockUser.joinedDate
  const level = Number(user?.user_metadata?.level || sourceMockUser.level)
  const xp = Number(user?.user_metadata?.xp || sourceMockUser.xp)
  const xpToNext = Number(user?.user_metadata?.xp_to_next || sourceMockUser.xpToNext)
  const isPremium =
  ...
  const stats = React.useMemo(() => {
    ...
    const meanScore = scoreValues.length ? Number((scoreValues.reduce((sum, score) => sum + score, 0) / scoreValues.length / 10).toFixed(1)) : 0
  ...
  const recentActivity = React.useMemo(() => {
    return entries
      ...
      .map((entry) => ({
        ...
        score: entry?.media?.averageScore ? Number((entry.media.averageScore / 10).toFixed(1)) : null,
  ...
  const mockUser = React.useMemo(() => ({
    name: displayName,
    username: normalizedHandle,
    avatar: avatarUrl || sourceMockUser.avatar,
    banner: bannerUrl || sourceMockUser.banner,
    bio,
    location,
    website,
    joinedDate,
    ...
    xpToNext,
    stats,
    recentActivity: recentActivity.length ? recentActivity : sourceMockUser.recentActivity,
    favorites: favorites.length
      ? favorites.slice(0, 4).map((fav) => ({
          title: getMediaTitle(fav),
          image: fav?.coverImage?.large || "",
          score: fav?.averageScore ? Number((fav.averageScore / 10).toFixed(1)) : null,
        }))
      : sourceMockUser.favorites,
        ...
        <div className="relative -mt-24 sm:-mt-28 mb-8">
            ...
            <div className="flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="relative group">
                <div className="relative">
                  {/* Glow ring */}
                  <div className="absolute -inset-1 bg-gradient-to-r from-primary via-accent to-primary rounded-2xl opacity-60 blur group-hover:opacity-100 transition-opacity" />
                  
                  {/* Avatar */}
                  <div className="relative w-36 h-36 sm:w-44 sm:h-44 rounded-2xl border-4 border-background overflow-hidden bg-muted shadow-2xl">
                    <img
                      src={mockUser.avatar}
                      alt={mockUser.name}
                      className="h-full w-full object-cover"
              ...
              <div className="flex-1 pt-4 sm:pt-8">
                ...
                <div className="mt-5 flex flex-wrap gap-2.5 xl:flex-nowrap">
                  {[
                    ...
                    { label: "Watched", value: mockUser.stats.daysWatched, icon: Clock, color: "text-cyan-400" },
                    { label: "Streak", value: mockUser.stats.currentStreak, icon: Flame, color: "text-orange-400" },
                    { label: "Score", value: mockUser.stats.meanScore, icon: Star, color: "text-amber-400" },
                  ].map((stat, i) => (
                    <motion.div
                      key={stat.label}
          ...
          {mockUser.bio && (
            <motion.p 
              ...
              className="mt-6 text-muted-foreground max-w-3xl leading-relaxed text-lg"
            >
              {mockUser.bio}
            </motion.p>
          )}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm text-muted-foreground mt-1">
                              {activity.status === "completed" ? "Completed" : `Episode ${activity.episode}`}
                            </p>
                          ...
                          <div className="flex flex-col items-end gap-2">
                            <Badge className={cn(
                              ...
                              {activity.status === "completed" ? "Completed" : "Watching"}
                            </Badge>
                            {activity.score && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {mockUser.favorites.map((fav, i) => (
                        <motion.div 
                          ...
                          transition={{ duration: 0.3, delay: i * 0.1 }}
                        >
                          <div className="relative aspect-[3/4] rounded-xl overflow-hidden shadow-lg group-hover:shadow-xl transition-all">
                            <img
                              src={fav.image}
                              alt={fav.title}
                              className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110"
                            ...
                            <div className="absolute bottom-0 left-0 right-0 p-3">
                              <p className="text-xs font-semibold text-white line-clamp-2">{fav.title}</p>
                            </div>
                          </div>
                        </motion.div>
                      ))}
...
<div className="w-10 h-10 flex items-center justify-center mx-auto mb-2">
                  ...
                  <div className="bg-gradient-to-br from-primary/10 via-accent/5 to-primary/10 border border-primary/20 rounded-2xl p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <p className="text-2xl font-black text-foreground">2.6</p>
                        <p className="text-xs text-muted-foreground">Days Watched</p>
                      </div>
                      <div>
                        <p className="text-2xl font-black text-foreground">8.7</p>
